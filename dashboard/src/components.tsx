...
> {
  ...
  render() {
    return this.state.failed ? (
      <ReadFailure
        message="This view could not be displayed. Retry or navigate to another section; recorded evidence is unchanged."
        correlationId={null}
  ...
  return (
    <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <h1 className="text-balance text-[1.75rem]/9 font-semibold tracking-tight text-ink">
          {title}
        </h1>
        <p className="mt-2 max-w-[72ch] text-pretty text-base/6 text-muted">
          {description}
        </p>
      </div>
      {actions ? (
...
export function Notice({
  ...
  return (
    <section
      className={`rounded-panel border p-4 ${colors[tone]}`}
...
export function ReadFailure({
  ...
  return (
    <Notice title="Could not read this information" tone="danger">
      <p>{message}</p>
      {correlationId ? (
        <p className="mt-2 font-mono text-sm/5" translate="no">
        ...
        return (
          ...
          >
            ...
            {label === "Attention" && attentionCount ? (
              <span
                className="ml-auto rounded-full bg-warning-soft px-1.5 font-mono text-sm/5 text-warning tabular-nums"
                aria-label={`${attentionCount} attention records`}
              >
                {attentionCount}
              </span>
            ) : null}
...
function MobileNavigation({
  ...
  return (
    <>
      <button
        className="secondary-action px-3 text-sm/5"
        type="button"
...
export function Shell({
  ...
  return (
    <div className="isolate min-h-dvh bg-canvas antialiased">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus-ring absolute left-4 top-4 z-50 rounded-control bg-surface px-3 py-2 text-sm/5 text-ink"
      >
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-[13.5rem] border-r border-border bg-canvas p-5 min-[1200px]:block">
        <a
          href="/"
          className="mb-8 flex items-center gap-2 text-ink no-underline"
          aria-label="Homepage"
        >
          <BoltIcon
            aria-hidden="true"
            className="size-4 shrink-0 fill-primary"
          />
          <span className="text-base/6 font-semibold">Backend Ops</span>
        </a>
        <Navigation pathname={pathname} attentionCount={attentionCount} />
        <p className="absolute bottom-5 left-5 right-5 text-sm/5 text-muted">
          Local dissertation testbed
        </p>
      </aside>
      ...
      <main
        id="main-content"
        className="min-w-0 p-5 sm:p-6 min-[1200px]:ml-[13.5rem] min-[1200px]:p-8"
        tabIndex={-1}
      >
...
export function DetailLink({
  ...
  return (
    <a
      className="inline-flex min-h-11 items-center gap-1 text-sm/5 font-medium text-primary underline decoration-primary/30 underline-offset-4 hover:decoration-primary"
      href={href}
    >
      {children}
      <ChevronRightIcon
