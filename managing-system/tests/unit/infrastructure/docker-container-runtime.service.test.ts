...
import { DockerContainerRuntimeService, type ExecFileImplementation } from "@/infrastructure/container-runtime";
...
test("Docker runtime maps fixed targets through argument-based execution", async () => {
  ...
  const runtime = new DockerContainerRuntimeService({ dockerEnabled: true, dockerTimeoutMs: 50 }, executeFile);
...
test("Docker runtime rejects disabled, unknown and inherited targets before process execution", async () => {
  ...
  const disabled = new DockerContainerRuntimeService({ dockerEnabled: false, dockerTimeoutMs: 10 }, executeFile);
...
test("Docker runtime terminates a timed-out process", async () => {
  ...
  const runtime = new DockerContainerRuntimeService(
    ...
    () => ({ kill() { killed = true; return true; } } as never),
  );
