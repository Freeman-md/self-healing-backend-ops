...
export async function injectFaultProfile(profileCode: FaultProfileCode): Promise<void> {
  if (profileCode === "managed_system_application_network_isolated") {
    throw new Error("Isolation requires the registered network-isolation lifecycle.");
  }

  for (const target of findFaultProfile(profileCode).stoppedTargets) {
    await executeFile("docker", ["stop", containerNames[target]], { timeout: 10_000 });
  }
}

export async function restoreExperimentTargets(): Promise<void> {
  await executeFile("docker", ["restart", containerNames.postgres], { timeout: 10_000 });
  await executeFile("docker", ["restart", containerNames["managed-system"]], { timeout: 10_000 });
}
...
export class ApplicationNetworkIsolation {
  ...
  private async inspect(name: string) {
    // This projection deliberately excludes container environment values.
    const format =
  ...
  async captureAttachment(): Promise<NetworkAttachment> {
    const [app, database] = await Promise.all([
      this.inspect("managed-system-app"),
      this.inspect("managed-system-postgres"),
    ]);
  ...
  async disconnectApplication(input: NetworkAttachment): Promise<void> {
    const attachment = attachmentSchema.parse(input);

    const current = await this.captureAttachment();

    if (JSON.stringify(current) !== JSON.stringify(attachment)) {
...
export async function assertUnreachable(baseUrl: string): Promise<void> {
  try {
    await fetch(new URL("/health", baseUrl), { signal: AbortSignal.timeout(5_000) });
  } catch {
    return;
  }
