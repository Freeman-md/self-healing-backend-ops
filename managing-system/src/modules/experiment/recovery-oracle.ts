...
import type { RecoveryOracleResult } from "./experiment.types";
...
export class RecoveryOracle {
    ...
    while (Date.now() - startedAt <= stabilityWindowMs) {
      ...
      if (!observation.healthy) {
        return {
          ...
          details: { ...details, stabilityWindowMs, stableForMs: Date.now() - startedAt },
        };
      }
  ...
  private async observe(): Promise<{ healthy: boolean; details: Record<string, unknown> }> {
  ...
  private async fetchHealth(): Promise<{
    ...
    try {
      ...
      return { reachable: false, status: null, appStatus: null, databaseStatus: null };
    }
  }
